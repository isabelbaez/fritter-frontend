...
router.post(
  ...
  [
    ...
    commentValidator.isValidCommentParent
  ],
  async (req: Request, res: Response) => {
    const userId = (req.session.userId as string) ?? ''; // Will not be an empty string since its validated in isUserLoggedIn
    const comment = await CommentCollection.addOne(userId, req.body.parentId, req.body.content);
 ...
 * @return {CommentResponse} - the updated freet
